...
class ProcurementStore:
    ...
    @staticmethod
    def _event(connection: Connection, update_id: str, event: str, actor_user_id: str | None, reason: str | None = None) -> str:
        event_id = str(uuid4())
        connection.execute(
            "INSERT INTO procurement_update_events (id, update_id, event, actor_user_id, reason, created_at) VALUES (%s, %s, %s, %s, %s, %s)",
            (event_id, update_id, event, actor_user_id, reason, datetime.now(UTC).isoformat()),
        )

        connection.execute("UPDATE procurement_updates SET updated_at=%s WHERE id=%s", (datetime.now(UTC).isoformat(), update_id))
        return event_id

    @staticmethod
    def _editable_update(connection: Connection) -> tuple[Any, ...] | None:
    ...
    def _ensure_editable_update(
        ...
        if current:
            ...
            connection.execute(
                """UPDATE procurement_updates SET source_name = %s, status = 'draft', updated_at = %s,
    ...
    def confirm_import(self, source_name: str, effective_date: date, content: str, actor_user_id: str, price_confirmation: PriceConfirmation | None = None, reason: str = "已核对导入报价及价格波动", *, revision: tuple | None = None) -> dict[str, Any]:
        ...
        with transaction(self.url, write=True) as connection:
            ...
            collaboration.require_current(self.url, actor_user_id, "can_edit")
            if price_confirmation is not None:
                collaboration.require_current(self.url, actor_user_id, "can_activate")
            ...
            for row in preview["rows"]:
                ...
                before = connection.execute("SELECT latest_price FROM procurement_update_items WHERE update_id=%s AND material_id=%s", (update_id, material_id)).fetchone()
                if before is None:
                    before = connection.execute("SELECT latest_price FROM procurement_price_batch_items WHERE batch_id=%s AND material_id=%s", (self._latest_batch_id(connection), material_id)).fetchone()
                ...
                same_period = connection.execute(
                    """SELECT m.previous_latest_price FROM procurement_materials m JOIN procurement_imports i ON i.id=m.last_import_id
                       WHERE m.id=%s AND i.effective_date=%s""", (material_id, effective_date.isoformat()),
                ).fetchone()
                if same_period:
                    previous_latest = same_period[0]
                connection.execute(
                    """UPDATE procurement_materials SET latest_price = %s, inventory_price = %s,
                              in_transit_price = %s, previous_latest_price = %s, last_import_id = %s, updated_at = %s
                       WHERE id = %s""",
                    (row["latest_price"], row["inventory_price"], row["in_transit_price"], previous_latest, import_id, now, material_id),
                )
                connection.execute(
                    "INSERT INTO procurement_price_history (id, material_id, import_id, latest_price, inventory_price, in_transit_price, recorded_at) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                    (
                        str(uuid4()),
                        material_id,
                        import_id,
            ...
            connection.execute(
                "UPDATE procurement_working_state SET latest_import_id = %s, submitted_by = NULL, submitted_at = NULL WHERE id = 1",
                (import_id,),
            )
    ...
    def overview(self) -> dict[str, Any]:
        with transaction(self.url) as connection:
            ...
            published_rows = connection.execute(
                "SELECT material_id, latest_price FROM procurement_price_batch_items WHERE batch_id = %s",
                (current_batch_id,),
            ...
            sources = collaboration.snapshot_sources(self.url, current_batch_id)
            all_participants = collaboration.participants(self.url)
            round_participants = collaboration.participants(self.url, active_update["id"]) if active_update else {}
            pending = active_update or scheduled_update
            authorship = collaboration.price_authorship(self.url)
                ...
                "batches": [
                    {
                        ...
                        "provenance": collaboration.provenance(self.url, str(row[0])),
    ...
    def _user_name(self, user_id: str) -> str:
        with transaction(self.url) as connection:
            row = connection.execute("SELECT display_name FROM identity_users WHERE id=%s", (user_id,)).fetchone()
        return str(row[0]) if row else "未记录"

    def _batch_update_id(self, batch_id: str) -> str | None:
        with transaction(self.url) as connection:
            row = connection.execute("SELECT update_id FROM procurement_price_batches WHERE id=%s", (batch_id,)).fetchone()
    ...
    def get_update(self, update_id: str) -> dict[str, Any] | None:
        with transaction(self.url) as connection:
            ...
            input_ids = {str(row[0]) for row in connection.execute(
                """SELECT material_id FROM procurement_update_items WHERE update_id = %s
                   ...
                   WHERE i.update_id = %s AND i.archived_at IS NULL AND a.id IS NULL""",
                (update_id, update_id),
            ).fetchall()}
            issue_rows = connection.execute(
                """SELECT issues.id, issues.material_id, materials.code, materials.name, issues.kind,
                   ...
                   FROM procurement_issues AS issues
                   JOIN procurement_materials AS materials ON materials.id = issues.material_id
                   WHERE issues.update_id = %s AND issues.status != 'resolved'
            ...
            event_rows = connection.execute(
                """SELECT events.event, COALESCE(users.display_name, events.actor_user_id),
                   ...
                   WHERE events.update_id = %s ORDER BY events.created_at DESC""",
                (update_id,),
            ).fetchall()
            items = [
                {
            ...
            return {
                ...
                "changes": collaboration.saved_events(self.url, update_id=update_id),
                "issues": issues,
                "events": [
        ...
        with transaction(self.url, write=True) as connection:
            ...
            error_count = int(connection.execute(
                """SELECT COUNT(*) FROM procurement_issues
                   WHERE update_id = %s AND status = 'open' AND kind != 'price_spike'""",
                (update_id,),
            ).fetchone()[0])
            if error_count:
                raise ValueError(f"仍有 {error_count} 个数据错误，不能提交")
            connection.execute(
                "UPDATE procurement_working_state SET submitted_by = %s, submitted_at = %s WHERE id = 1",
                (actor_user_id, now),
            )
            connection.execute(
                """UPDATE procurement_updates SET status = 'submitted', submitted_by = %s,
                   submitted_at = %s, updated_at = %s WHERE id = %s""",
    ...
    def review_issue(
        ...
        with transaction(self.url, write=True) as connection:
            ...
            if updated_at is not None:
                revision = connection.execute("SELECT updated_at FROM procurement_updates WHERE id=%s", (issue[2],)).fetchone()
            ...
            if str(issue[1]) == "open":
                connection.execute(
                    """UPDATE procurement_issues SET status = 'reviewed', reviewed_by = %s,
                       reviewed_at = %s, review_reason = %s WHERE id = %s""",
    ...
    def return_update(self, update_id: str, actor_user_id: str, reason: str) -> dict[str, Any]:
        ...
        with transaction(self.url, write=True) as connection:
            ...
            changed = connection.execute(
                """UPDATE procurement_updates SET status = 'returned', return_reason = %s, updated_at = %s
    ...
    def cancel_update(self, update_id: str, actor_user_id: str, reason: str) -> dict[str, Any]:
        now = datetime.now(UTC).isoformat()
        with transaction(self.url, write=True) as connection:
            collaboration.require_current(self.url, actor_user_id, "can_cancel_round")
            self._freeze_update(connection, update_id)
            changed = connection.execute(
                """UPDATE procurement_updates SET status = 'cancelled', cancelled_by = %s,
                   cancelled_at = %s, cancel_reason = %s, updated_at = %s
                   WHERE id = %s AND status IN ('draft', 'returned')""",
            ...
            connection.execute(
                "UPDATE procurement_issues SET status = 'resolved' WHERE update_id = %s AND status = 'open'",
                (update_id,),
            )
            self._event(connection, update_id, "cancelled", actor_user_id, reason.strip())
            for (material_id,) in connection.execute("SELECT material_id FROM procurement_update_items WHERE update_id=%s", (update_id,)).fetchall():
    ...
    def material_detail(self, material_id: str, include_archived: bool = False) -> dict[str, Any] | None:
        with transaction(self.url) as connection:
            ...
            material = connection.execute(
                "SELECT id, code, name, unit, archived_at, updated_at, latest_price FROM procurement_materials WHERE id = %s",
                (material_id,),
            ).fetchone()
            adjustments = connection.execute(
                """SELECT adjustments.id, adjustments.target_history_id, adjustments.replacement_history_id,
                   ...
                   WHERE adjustments.material_id = %s ORDER BY adjustments.created_at DESC""",
                (material_id,),
            ).fetchall()
            official = connection.execute("""SELECT b.id,b.version,b.price_date,i.latest_price,
                ...
                WHERE i.material_id=%s ORDER BY b.version""", (material_id,)).fetchall()
            sources = connection.execute("""SELECT s.sheet,s.source_row,s.raw_json,f.filename
                ...
                WHERE s.material_id=%s ORDER BY s.sheet,s.source_row""", (material_id,)).fetchall()
            if material is None or (material[4] is not None and not include_archived):
                return None
            authorship = collaboration.price_authorship(self.url, material_id)["batches"]
    ...
    def preferences(self, user_id: str) -> dict[str, Any]:
        ...
        with transaction(self.url) as connection:
            row = connection.execute(
                "SELECT ledger_columns, history_view, ledger_view, ledger_page_size FROM procurement_user_preferences WHERE user_id = %s",
                (user_id,),
            ).fetchone()
        return {
    ...
    def save_preferences(self, user_id: str, ledger_columns: list[str], history_view: str, ledger_view: str, ledger_page_size: int) -> dict[str, Any]:
        ...
        now = datetime.now(UTC).isoformat()
        with transaction(self.url, write=True) as connection:
            user = IdentityStore(self.url).get_user(user_id)
            if not user or not user["is_active"] or not (user["is_system_admin"] or user["scope_levels"].get("procurement", 0) >= 2):
    ...
    def update_material_identity(self, material_id: str, code: str, name: str, actor_user_id: str) -> dict[str, Any]:
        ...
        now = datetime.now(UTC).isoformat()
        with transaction(self.url, write=True) as connection:
            collaboration.require_current(self.url, actor_user_id, "can_manage_catalog")
            material = connection.execute(
                "SELECT code, name FROM procurement_materials WHERE id = %s AND archived_at IS NULL",
                (material_id,),
            ).fetchone()
            if material is None:
            ...
            if code != old_code:
                current = connection.execute(
                    "SELECT id FROM procurement_materials WHERE code = %s AND id != %s",
                    (code, material_id),
                ).fetchone()
                alias = connection.execute(
                    "SELECT material_id FROM procurement_material_aliases WHERE alias_code = %s",
                    (code,),
                ).fetchone()
                if current is not None or (alias is not None and str(alias[0]) != material_id):
                    raise RuntimeError("原料编号已被使用")
                if alias is not None:
                    connection.execute("DELETE FROM procurement_material_aliases WHERE alias_code = %s", (code,))
                connection.execute(
                    "INSERT INTO procurement_material_aliases (alias_code, material_id, created_by, created_at) VALUES (%s, %s, %s, %s) ON CONFLICT DO NOTHING",
                    (old_code, material_id, actor_user_id, now),
                )
            connection.execute(
                "UPDATE procurement_materials SET code = %s, name = %s, updated_at = %s WHERE id = %s",
                (code, name, now, material_id),
            )
            if code != old_code or name != old_name:
                connection.execute(
                    "INSERT INTO procurement_material_identity_changes (id, material_id, old_code, new_code, old_name, new_name, created_by, created_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
    ...
    def create_material(self, payload: MaterialCreateRequest, actor_user_id: str) -> str:
        ...
        with transaction(self.url, write=True) as connection:
            collaboration.require_current(self.url, actor_user_id, "can_manage_catalog")
            if connection.execute("SELECT 1 FROM procurement_materials WHERE code=%s UNION ALL SELECT 1 FROM procurement_material_aliases WHERE alias_code=%s LIMIT 1", (code, code)).fetchone():
            ...
            if payload.price is not None:
                ...
                current = connection.execute("SELECT status FROM procurement_updates WHERE id=%s", (payload.update_id,)).fetchone()
            ...
            connection.execute("INSERT INTO procurement_materials(id,code,name,unit,updated_at) VALUES (%s,%s,%s,'kg',%s)", (material_id, code, payload.name.strip() or code, datetime.now(UTC).isoformat()))
            connection.cursor().executemany("INSERT INTO procurement_department_materials (department_id, material_id) VALUES (%s,%s)", [(group, material_id) for group in groups])
    ...
    def adjust_price(self, material_id: str, price: Decimal, effective_date: date, reason: str, actor_user_id: str, target_history_id: str | None, price_confirmation: PriceConfirmation | None = None, *, updated_at: str | None = None) -> dict[str, Any]:
        with transaction(self.url, write=True) as connection:
            ...
            if price_confirmation is not None:
                collaboration.require_current(self.url, actor_user_id, "can_activate")
            if updated_at is not None:
                material = connection.execute("SELECT updated_at FROM procurement_materials WHERE id=%s", (material_id,)).fetchone()
    ...
    def _adjust_price(self, connection: Connection, material_id: str, price: Decimal, effective_date: date, reason: str, actor_user_id: str, target_history_id: str | None, record_event: bool = True) -> dict[str, Any]:
        now = datetime.now(UTC).isoformat()
        ...
        material = connection.execute("SELECT id FROM procurement_materials WHERE id = %s AND archived_at IS NULL", (material_id,)).fetchone()
        if material is None:
            raise LookupError("Material not found")
        active = self._active_history(connection=connection, material_id=material_id)
        before_row = connection.execute("SELECT latest_price FROM procurement_update_items WHERE update_id=%s AND material_id=%s", (update_id, material_id)).fetchone()
        if before_row is None:
            before_row = connection.execute("SELECT latest_price FROM procurement_price_batch_items WHERE batch_id=%s AND material_id=%s", (self._latest_batch_id(connection), material_id)).fetchone()
        ...
        connection.execute(
            ...
            ) VALUES (%s, '手工价格修正', 1, 1, 0, %s, %s, %s, NULL, NULL, %s)""",
        ...
        if target_history_id and connection.execute("SELECT 1 FROM procurement_price_adjustments WHERE target_history_id=%s", (target_history_id,)).fetchone():
    ...
    def bulk_adjust_prices(self, payload: BulkPriceAdjustmentRequest, actor_user_id: str) -> str:
        with transaction(self.url, write=True) as connection:
            ...
            for item in payload.items:
                before = connection.execute("SELECT latest_price FROM procurement_update_items WHERE update_id=%s AND material_id=%s", (payload.update_id, item.material_id)).fetchone()
                if before is None:
                    before = connection.execute("SELECT latest_price FROM procurement_price_batch_items WHERE batch_id=%s AND material_id=%s", (self._latest_batch_id(connection), item.material_id)).fetchone()
                changes.append((item.material_id, before[0] if before else None, _decimal_text(item.price)))
                material = connection.execute("SELECT code FROM procurement_materials WHERE id=%s", (item.material_id,)).fetchone()
                try:
    ...
    def preview_prices(self, payload: PricePreviewRequest) -> dict[str, Any]:
        with transaction(self.url) as connection:
            ...
            rows = []
            for item in payload.items:
                material = connection.execute("SELECT code, name FROM procurement_materials WHERE id=%s AND archived_at IS NULL", (item.material_id,)).fetchone()
                if material is None:
                    raise LookupError("原料不存在")
                reference = self._reference_for_date(connection, payload.effective_date.isoformat(), item.material_id)
                official = connection.execute("SELECT 1 FROM procurement_price_batch_items WHERE batch_id=%s AND material_id=%s", (baseline_id, item.material_id)).fetchone()
    ...
    def _confirm_saved_risks(self, connection: Connection, update_id: str, changed_ids: set[str], confirmation: PriceConfirmation, reason: str, actor_user_id: str) -> None:
        if not 4 <= len(reason.strip()) <= 200:
        ...
        now = datetime.now(UTC).isoformat()
        for material_id in changed_ids:
            changed = connection.execute("""UPDATE procurement_issues SET status='reviewed', reviewed_by=%s, reviewed_at=%s, review_reason=%s
                WHERE update_id=%s AND material_id=%s AND kind='price_spike' AND status='open'""", (actor_user_id, now, reason.strip(), update_id, material_id)).rowcount
            if changed:
            ...
            if kind == "imports":
                pending = connection.execute(
                    """SELECT 1 FROM procurement_imports i JOIN procurement_updates u ON u.id=i.update_id
                       WHERE i.id=%s AND u.status IN ('draft','returned','submitted','scheduled','revalidation_required')""", (item_id,),
                ).fetchone()
                if pending:
                    raise ValueError("请先完成或取消该批次，再归档或恢复其询价记录")
            changed = connection.execute(
                f"UPDATE {table} SET archived_at = %s, archived_by = %s WHERE id = %s",
                (now if archived else None, actor_user_id if archived else None, item_id),
            ).rowcount
            if kind == "imports":
                material_ids = [str(row[0]) for row in connection.execute("SELECT DISTINCT material_id FROM procurement_price_history WHERE import_id = %s", (item_id,))]
                for material_id in material_ids:
                    self._refresh_material_snapshot(connection, material_id)
        return bool(changed)

    def _active_history(self, *, include_archived: bool = False, material_id: str | None = None, connection: Connection | None = None) -> list[dict[str, Any]]:
        if connection is None:
            with transaction(self.url) as db:
                return self._active_history(include_archived=include_archived, material_id=material_id, connection=db)
        ...
        if material_id:
            where.append("history.material_id = %s")
        ...
        rows = connection.execute(
            f"""SELECT history.id, history.material_id, materials.code, materials.name, materials.unit,
                ...
                ORDER BY imports.effective_date, materials.code, imports.created_at, imports._order, history._order""",
            params,
        ).fetchall()
    ...
    def _refresh_material_snapshot(self, connection: Connection, material_id: str) -> None:
        rows = self._active_history(connection=connection, material_id=material_id)
        rows.sort(key=lambda item: (item["effective_date"], item["created_at"]))
        latest = rows[-1] if rows else None
        previous = rows[-2] if len(rows) > 1 else None
        baseline = connection.execute("SELECT latest_price FROM procurement_price_batch_items WHERE batch_id=%s AND material_id=%s", (self._latest_batch_id(connection), material_id)).fetchone()
        connection.execute(
            """UPDATE procurement_materials SET latest_price = %s, inventory_price = %s, in_transit_price = %s,
               previous_latest_price = %s, last_import_id = %s, updated_at = %s WHERE id = %s""",
    ...
    @staticmethod
    def _history_import_id(connection: Connection, history_id: str) -> str:
        return str(connection.execute("SELECT import_id FROM procurement_price_history WHERE id = %s", (history_id,)).fetchone()[0])

    @staticmethod
    def _latest_batch_id(connection: Connection) -> str | None:
        row = connection.execute(
            "SELECT id FROM procurement_price_batches ORDER BY version DESC LIMIT 1"
        ).fetchone()
        return str(row[0]) if row else None

    def _candidate_snapshot(self, connection: Connection, update_id: str) -> list[tuple[Any, ...]]:
        update = connection.execute("SELECT status FROM procurement_updates WHERE id = %s", (update_id,)).fetchone()
        ...
        rows = {row[0]: row for row in connection.execute(
            """SELECT m.id, m.code, m.name, m.unit, b.latest_price, b.inventory_price,
               ...
               FROM procurement_materials m LEFT JOIN procurement_price_batch_items b
                 ON b.material_id = m.id AND b.batch_id = %s
        ...
        history = connection.execute(
            """SELECT m.id, m.code, m.name, m.unit, h.latest_price, h.inventory_price, h.in_transit_price, NULL
               ...
               LEFT JOIN procurement_price_adjustments a ON a.target_history_id = h.id
               WHERE i.update_id = %s AND i.archived_at IS NULL AND m.archived_at IS NULL AND a.id IS NULL
               ORDER BY i.created_at, i._order, h._order""", (update_id,),
        ).fetchall()
    ...
    @staticmethod
    def _save_candidate(connection: Connection, update_id: str, material_id: str, history_id: str | None = None) -> None:
        material = connection.execute(
            "SELECT id, code, name, unit, latest_price, inventory_price, in_transit_price FROM procurement_materials WHERE id = %s",
            (material_id,),
        ).fetchone()
        if history_id:
            values = connection.execute("SELECT latest_price, inventory_price, in_transit_price FROM procurement_price_history WHERE id = %s", (history_id,)).fetchone()
            material = (*material[:4], *values)
        connection.execute(
            """INSERT INTO procurement_update_items (update_id, material_id, code, name, unit, latest_price, inventory_price, in_transit_price, recommended_price) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
    ...
    def _reference_price(self, connection: Connection, update_id: str, material_id: str) -> str | None:
        price_date = connection.execute("SELECT price_date FROM procurement_updates WHERE id=%s", (update_id,)).fetchone()[0]
        return self._reference_for_date(connection, str(price_date), material_id)

    def _reference_for_date(self, connection: Connection, price_date: str, material_id: str) -> str | None:
        official = connection.execute(
            "SELECT latest_price FROM procurement_price_batch_items WHERE batch_id = %s AND material_id = %s",
        ...
        previous = connection.execute(
            """SELECT h.latest_price FROM procurement_price_history h JOIN procurement_imports i ON i.id=h.import_id
               LEFT JOIN procurement_price_adjustments a ON a.target_history_id=h.id
               WHERE h.material_id=%s AND i.archived_at IS NULL AND a.id IS NULL
                 AND i.effective_date < %s
               ORDER BY i.effective_date DESC, i.created_at DESC, i._order DESC LIMIT 1""",
            (material_id, price_date),
        ).fetchone()
        if previous:
            return previous[0]
        initial = connection.execute(
            "SELECT previous_latest_price FROM procurement_materials WHERE id=%s", (material_id,),
        ).fetchone()
        return initial[0] if initial else None

    def _refresh_update_issues(self, connection: Connection, update_id: str, now: str, changed_ids: set[str] | None = None) -> None:
        for row in self._candidate_snapshot(connection, update_id):
            ...
            for kind, present in required.items():
                existing = connection.execute(
                    "SELECT status FROM procurement_issues WHERE update_id=%s AND material_id=%s AND kind=%s AND status!='resolved'",
    ...
    def _validate_snapshot(self, connection: Connection, update_id: str) -> None:
        ...
        for row in rows:
            if _has_price_spike(_decimal(row[4]), _decimal(self._reference_price(connection, update_id, str(row[0])))):
                confirmed = connection.execute(
                    "SELECT 1 FROM procurement_issues WHERE update_id=%s AND material_id=%s AND kind='price_spike' AND status='reviewed' AND length(trim(review_reason)) BETWEEN 4 AND 200",
                    (update_id, row[0]),
                ).fetchone()
    ...
    @staticmethod
    def _frozen_snapshot(connection: Connection, update_id: str) -> list[tuple[Any, ...]]:
        return connection.execute(
            """SELECT material_id, code, name, unit, latest_price, inventory_price,
                      in_transit_price, recommended_price
               FROM procurement_update_items WHERE update_id = %s ORDER BY code""",
            (update_id,),
        ).fetchall()

    def _freeze_update(self, connection: Connection, update_id: str) -> list[tuple[Any, ...]]:
        rows = self._candidate_snapshot(connection, update_id)
        collaboration.freeze_sources(connection, update_id, update_id, self._latest_batch_id(connection))
        connection.execute("DELETE FROM procurement_update_items WHERE update_id = %s", (update_id,))
    ...
    def _refresh_revalidation_issues(self, connection: Connection, update_id: str, now: str) -> None:
        self._refresh_update_issues(connection, update_id, now)

    def _activate_update(self, connection: Connection, update_id: str, actor_user_id: str, now: str) -> dict[str, Any]:
        update = connection.execute(
            """SELECT price_date, source_name, submitted_by, base_batch_id, status
               FROM procurement_updates WHERE id = %s""",
            (update_id,),
        ).fetchone()
        ...
        connection.cursor().executemany(
            "INSERT INTO procurement_price_batch_items (batch_id, material_id, code, name, unit, latest_price, inventory_price, in_transit_price, recommended_price) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
            [
                (
        ...
        previous_id = connection.execute("SELECT id FROM procurement_price_batches WHERE version<%s ORDER BY version DESC LIMIT 1", (version,)).fetchone()
        collaboration.freeze_sources(connection, batch_id, update_id, previous_id[0] if previous_id else None)
        connection.execute(
            """UPDATE procurement_updates SET status = 'published', published_batch_id = %s,
        ...
        scheduled = connection.execute(
            "SELECT id FROM procurement_updates WHERE status = 'scheduled' AND id != %s",
            (update_id,),
        ).fetchall()
        for scheduled_row in scheduled:
            scheduled_id = str(scheduled_row[0])
            connection.execute(
                "UPDATE procurement_updates SET status = 'revalidation_required', updated_at = %s WHERE id = %s",
                (now, scheduled_id),
            )
        ...
        for (draft_id,) in connection.execute("SELECT id FROM procurement_updates WHERE status IN ('draft','returned','submitted','revalidation_required') AND id != %s", (update_id,)).fetchall():
    ...
    def publish_update(
        ...
        with transaction(self.url, write=True) as connection:
            collaboration.require_current(self.url, actor_user_id, "can_activate")
            if revision is not None:
                row = connection.execute("SELECT updated_at FROM procurement_updates WHERE id=%s", (update_id,)).fetchone()
                if not row or (row[0], self._latest_batch_id(connection)) != revision:
                    raise RuntimeError("启用概况已变化，请刷新核对后重新确认")
            update = connection.execute(
                "SELECT status, submitted_by FROM procurement_updates WHERE id = %s",
                (update_id,),
            ).fetchone()
            ...
            open_issue_count = int(connection.execute(
                "SELECT COUNT(*) FROM procurement_issues WHERE update_id = %s AND status = 'open'",
                (update_id,),
            ).fetchone()[0])
            if open_issue_count:
            ...
            if str(update[0]) in {"draft", "returned", "submitted"}:
                self._freeze_update(connection, update_id)
            connection.execute(
                """UPDATE procurement_updates SET status = 'scheduled', scheduled_activate_at = %s,
                   base_batch_id = %s, updated_at = %s WHERE id = %s""",
        ...
        with transaction(self.url, write=True) as connection:
            rows = connection.execute(
                """SELECT id, base_batch_id FROM procurement_updates
                   WHERE status = 'scheduled' AND scheduled_activate_at <= %s ORDER BY scheduled_activate_at""",
                (now_dt.isoformat(),),
            ).fetchall()
            for row in rows:
                update_id = str(row[0])
                approver = connection.execute("SELECT actor_user_id FROM procurement_update_events WHERE update_id=%s AND event='scheduled' ORDER BY created_at DESC,_order DESC LIMIT 1", (update_id,)).fetchone()
                if row[1] != self._latest_batch_id(connection) or not approver or not collaboration.can_activate(self.url, str(approver[0])):
                    connection.execute(
                        "UPDATE procurement_updates SET status = 'revalidation_required', updated_at = %s WHERE id = %s AND status = 'scheduled'",
    ...
    def cancel_schedule(self, update_id: str, actor_user_id: str, reason: str, copy_to_draft: bool) -> dict[str, Any]:
        ...
        with transaction(self.url, write=True) as connection:
            collaboration.require_current(self.url, actor_user_id, "can_activate")
            update = connection.execute(
                "SELECT price_date, source_name FROM procurement_updates WHERE id = %s AND status IN ('scheduled', 'revalidation_required')",
                (update_id,),
            ).fetchone()
            if update is None:
            ...
            connection.execute(
                """UPDATE procurement_updates SET status = 'cancelled', cancelled_by = %s,
                   cancelled_at = %s, cancel_reason = %s, updated_at = %s WHERE id = %s""",
                (actor_user_id, now, reason.strip(), now, update_id),
            )
            connection.execute(
                "UPDATE procurement_issues SET status = 'resolved' WHERE update_id = %s AND status = 'open'",
            ...
            if copy_to_draft:
                ...
                connection.execute(
                    """INSERT INTO procurement_update_items (update_id, material_id, code, name, unit, latest_price, inventory_price, in_transit_price, recommended_price) SELECT %s, material_id, code, name, unit,
                       latest_price, inventory_price, in_transit_price, recommended_price
                       FROM procurement_update_items WHERE update_id=%s""", (copied_id, update_id),
                )
                ...
                changes = connection.execute("""SELECT c.material_id,b.latest_price,c.after_price,c.price_date FROM procurement_saved_changes c
                    LEFT JOIN procurement_price_batch_items b ON b.material_id=c.material_id AND b.batch_id=%s
                    WHERE c.update_id=%s AND c._order=(SELECT MAX(s._order) FROM procurement_saved_changes s WHERE s.update_id=c.update_id AND s.material_id=c.material_id)""", (self._latest_batch_id(connection),update_id)).fetchall()
    ...
    def batch_comparison(self, batch_id: str, *, preserve_catalog: bool = False) -> dict[str, Any]:
        with transaction(self.url) as connection:
            previous = connection.execute("SELECT id, version FROM procurement_price_batches WHERE version < (SELECT version FROM procurement_price_batches WHERE id=%s) ORDER BY version DESC LIMIT 1", (batch_id,)).fetchone()
            old = dict(connection.execute("SELECT material_id, latest_price FROM procurement_price_batch_items WHERE batch_id=%s", (previous[0] if previous else None,)).fetchall())
            current = connection.execute("SELECT material_id, latest_price FROM procurement_price_batch_items WHERE batch_id=%s", (batch_id,)).fetchall()
            batch = connection.execute("SELECT version,update_id FROM procurement_price_batches WHERE id=%s", (batch_id,)).fetchone()
            reported = {row[0] for row in connection.execute("SELECT material_id FROM procurement_saved_changes WHERE update_id=%s", (batch[1],))} if batch else set()
            ...
            current_sources = collaboration.snapshot_sources(self.url, batch_id)
            old_sources = collaboration.snapshot_sources(self.url, previous[0] if previous else None)
    ...
    def get_batch(self, batch_id: str) -> dict[str, Any] | None:
        with transaction(self.url) as connection:
            ...
            items = connection.execute(
                ...
                SELECT material_id, code, name, unit, latest_price, inventory_price,
                       in_transit_price, recommended_price
                FROM procurement_price_batch_items WHERE batch_id = %s ORDER BY code
                """,
                (batch_id,),
            ).fetchall()
            scheduled_activation = connection.execute(
                """SELECT 1 FROM procurement_update_events WHERE event='scheduled_activation'
                   AND update_id=(SELECT update_id FROM procurement_price_batches WHERE id=%s)""", (batch_id,),
            ).fetchone()
            return {
                "id": str(batch[0]),
                "version": int(batch[1]),
                ...
                "changes": collaboration.saved_events(self.url, update_id=self._batch_update_id(batch_id)) if self._batch_update_id(batch_id) else [],
                "provenance": collaboration.provenance(self.url, batch_id),
                "snapshot_sources": collaboration.snapshot_sources(self.url, batch_id),
    ...
    @staticmethod
    def _material_for_code(connection: Connection, code: str) -> tuple[Any, ...] | None:
        return connection.execute(
            """SELECT materials.id, materials.code, materials.latest_price, materials.unit
               ...
               WHERE materials.archived_at IS NULL AND (materials.code = %s OR aliases.alias_code = %s)
               LIMIT 1""",
            (code, code),
        ).fetchone()
    ...
    def _sync_issue(
        ...
        if present:
            existing = connection.execute(
                "SELECT id FROM procurement_issues WHERE material_id = %s AND kind = %s AND update_id = %s AND status != 'resolved'",
                (material_id, kind, update_id),
            ).fetchone()
            if existing:
                connection.execute(
                    """UPDATE procurement_issues SET status = 'open', reviewed_by = NULL,
                       reviewed_at = NULL, review_reason = NULL WHERE id = %s""",
                    (existing[0],),
                )
            else:
                connection.execute(
                    """INSERT INTO procurement_issues (
                       ...
                       ) VALUES (%s, %s, %s, 'open', %s, NULL, NULL, %s, NULL)""",
                    (str(uuid4()), material_id, kind, now, update_id),
                )
        else:
            connection.execute(
                """UPDATE procurement_issues SET status = 'resolved'
                   WHERE material_id = %s AND kind = %s AND update_id = %s AND status != 'resolved'""",
                (material_id, kind, update_id),
            )
...
def create_procurement_router(
    ...
    def require_activation(user, *, manage=False):
        ...
        if not collaboration.capabilities(store.url, user)[key]:
            raise HTTPException(status_code=403, detail="需要启用授权管理权限" if manage else "尚未获得价格启用授权")

    def require_catalog_management(user):
        if not collaboration.capabilities(store.url, user)["can_manage_catalog"]:
    ...
    def overview(request: Request, editor_id: str | None = None, editor_scope: str = "round") -> dict[str, Any]:
        with transaction(store.url) as connection:
            user = actor(request, 2)
            result = store.overview()
            result["capabilities"] = collaboration.capabilities(store.url, user)
            result["environment"] = settings.environment
            result["departments"] = collaboration.departments(store.url, result)
            result["editors"] = [{"id": person["id"], "name": person["display_name"]} for person in IdentityStore(store.url).list_users()
    ...
    def list_activation_grants(request: Request, offset: int = Query(0, ge=0), limit: int = Query(10, ge=1, le=50)):
        ...
        return collaboration.grants(store.url, offset=offset, limit=limit)
    ...
    def set_activation_grant(user_id: str, payload: ActivationGrantRequest, request: Request):
        ...
        require_activation(user, manage=True)
        try:
            collaboration.set_grant(store.url, user, user_id, payload.enabled, payload.manager)
        except PermissionError as error:
            raise HTTPException(status_code=403, detail=str(error)) from error
        except ValueError as error:
            raise HTTPException(status_code=422, detail=str(error)) from error
        return collaboration.grants(store.url)
    ...
    def set_department_materials(department_id: str, payload: DepartmentMaterialsRequest, request: Request):
        ...
        try:
            collaboration.set_department(store.url, department_id, payload.material_ids, user["id"])
        except ValueError as error:
            raise HTTPException(status_code=422, detail=str(error)) from error
    ...
    def material_detail(material_id: str, request: Request, include_archived: bool = False) -> dict[str, Any]:
        with transaction(store.url) as connection:
            ...
            result["changes"] = visible_payload({"changes": collaboration.saved_events(store.url, material_id=material_id)}, user)["changes"]
