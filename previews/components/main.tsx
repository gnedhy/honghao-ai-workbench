...
import { FormFooter, DashboardPanel, PageState, WorkbenchLoading } from "../../src/components/WorkbenchLayout";
...
function Examples() {
  ...
  return <main className="component-preview"><h1>组件与动效验收</h1><p>隔离示例，不访问业务数据。当前系统：{reduced ? "减少动态效果" : "标准动态效果"}。减少动态效果跟随系统设置。</p>
    ...
    {state==="加载"&&<WorkbenchLoading title="正在读取采购数据"/>}
    ...
    {state==="加载"&&<DashboardPanel><h2>局部数据加载</h2><WorkbenchLoading local title="正在读取研发配方详情"/></DashboardPanel>}
