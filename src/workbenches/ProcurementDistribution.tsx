...
import { DiscardChangesDialog } from "../components/SettingsDialog";
...
export function ProcurementDistribution({ onOpenMaterial, revision }: { onOpenMaterial: (id: string) => void; revision: number }) {
  ...
  const returnToList = () => { setConfirmReturn(false); setEditing(false); setSelected(null); setQuery(""); };
  ...
  return <section className={`${styles.section} ${styles.subpageSection}`}>
    ...
    <div className={`${styles.sectionHeading} ${styles.ledgerHeading}`}><div><h2>{group?.name ?? "数据分流"}</h2><p className={styles.sectionDescription}>{!data ? "正在读取价格版本" : data.batches[0] ? `最新价格版本 v${data.batches[0].version} · ${data.batches[0].price_date?.replaceAll("-", ".") ?? "日期未记录"}` : "尚无价格版本"}。启用后自动更新，草稿不影响部门价格。</p></div>{selected && <button className="secondary-button" disabled={busy} onClick={() => { if (dirty) setConfirmReturn(true); else returnToList(); }}>返回部门列表</button>}</div>
...
{!data ? !error && <p>正在读取价格版本…</p> : !selected ? <div className={styles.tableWrap}><table className={`${styles.table} ${styles.distributionTable}`}><thead><tr><th>部门</th><th>关联原料</th><th>价格状态</th><th>操作</th></tr></thead><tbody>{data.departments?.map(item => { const missing = Math.max(0, item.material_ids.length-item.priced); return <tr key={item.id}><td>{item.name}</td><td>{item.material_ids.length} 项</td><td><span className={missing ? styles.distributionMissing : styles.distributionStatus}>{!item.material_ids.length ? "未关联原料" : missing ? `待定价 ${missing} 项` : "已定价"}</span></td><td><div className={styles.distributionActions}><button className="secondary-button" onClick={() => setSelected(item.id)}>查看明细</button>{data.capabilities?.can_manage_catalog && <button className="secondary-button" onClick={() => {setIds([...item.material_ids]); setOriginalIds([...item.material_ids]); setEditing(true); setSelected(item.id);}}>调整范围</button>}</div></td></tr>; })}</tbody></table>{!data.departments?.length && <p>尚未建立部门原料清单。</p>}</div> : group && <>
      ...
      <div className={styles.tableWrap}><table className={styles.table}><thead><tr>{editing && <th>选择</th>}<th>编号</th><th>单位</th><th>价格</th><th>来源日期</th></tr></thead><tbody>{materials.map(item => <tr key={item.id}>{editing && <td><input className={styles.rangeCheckbox} type="checkbox" aria-label={`关联 ${item.code}`} checked={ids.includes(item.id)} disabled={busy} onChange={e => setIds(value => e.target.checked ? [...value,item.id] : value.filter(id => id!==item.id))} /></td>}<td>{editing ? <strong>{item.code}</strong> : <button className={styles.materialLink} type="button" aria-label={`查看原料 ${item.code} 详情`} onClick={() => onOpenMaterial(item.id)}><span><strong>{item.code}</strong></span><ChevronRight size={16} aria-hidden="true" /></button>}</td><td>{item.unit}</td><td>{item.published_price == null ? "— 未定价" : `¥${item.published_price}`}</td><td>{item.published_price_date ?? "—"}</td></tr>)}</tbody></table></div>
    </>}
  </section>;
}
...
export function ProcurementActivationGrants({ admin, onChanged }: { admin: boolean; onChanged: () => void }) {
  ...
  return <><p>采购管理权限已包含价格启用；下方开关仅配置独立授权。撤销独立授权不移除管理权限带来的启用权；失去有效启用权后，未执行排期将暂停。</p>{error && <p role="alert" className={styles.error}>{error}</p>}{!data ? <p>正在读取授权…</p> : <><div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>人员</th><th>独立启用授权</th>{admin && <th>管理授权</th>}</tr></thead><tbody>{data.users.map(user => <tr key={user.id}><td>{user.name}{user.role_granted && <small style={{ display: "block" }}>管理权限已包含启用权</small>}{!user.eligible && <span>账号或采购编辑权限不可用</span>}</td><td><input type="checkbox" aria-label={`${user.name}独立启用授权`} disabled={busy || (!user.eligible && !user.granted) || (!admin && user.manager)} checked={user.granted} onChange={e => void save(user.id,e.target.checked)} /></td>{admin && <td><input type="checkbox" aria-label={`${user.name}管理授权`} checked={user.manager} disabled={busy || !user.eligible} onChange={e => void save(user.id,true,e.target.checked)} /></td>}</tr>)}</tbody></table></div><h3>授权记录</h3><div className={styles.eventList}>{data.events.map(event => <div key={event.id}><strong>{event.actor} · {event.action === "grant.enabled" ? "授予启用权" : "撤销启用权"}</strong><small>{data.users.find(user => user.id===event.target_id)?.name ?? event.target_id} · {new Intl.DateTimeFormat("zh-CN",{timeZone:"Asia/Shanghai",dateStyle:"short",timeStyle:"short"}).format(new Date(event.created_at))}</small></div>)}</div></>}</>;
}
