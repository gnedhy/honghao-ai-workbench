...
这是企业 AI 中台 V0.1 的独立研发项目。当前阶段先由个人在 Windows 本地、小范围、低敏感数据环境中验证一条真实闭环，再决定是否扩展到 AI 团队。
...
2. V0.2 视觉方案确认后，搭建 React + Vite + TypeScript 前端与 FastAPI + SQLite 后端骨架。
3. 先接 Mock Provider，跑通状态、审批和恢复；再接一个 OpenAI 兼容模型接口。
