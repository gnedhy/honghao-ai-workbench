...
2026-09-09 当前本机正式入口为 `http://127.0.0.1:8000/`，环境为 `production`，数据目录为 `%LOCALAPPDATA%\HonghaoAI\production`（Codex 宿主可能映射到自身 LocalCache）。PR #57 已合并，正式服务已启动并通过健康、就绪检查及已登录的采购台账页面核验。采购域整体取自 8012 验收库；保留旧库的项目、会话、消息、任务及 gnedhy 原密码，不包含 8020 的试操作。8012、8020 和旧 `.data` 均不是现役正式库，暂留复核，等待最终汇报后的清场确认。完整恢复包已包含数据库、配置、采购原件、代码与构建产物，并完成独立恢复验证。部署证据和历史边界见 [发布审查记录](./docs/发布审查-2026-09-09.md)。端口本身不能证明环境性质。
