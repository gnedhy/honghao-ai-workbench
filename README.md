...
这是以职能工作台为核心的本地 Web 应用，产品界面名为“宏昊 AI”。当前已启用采购原料价格管理及研发五部产品成本工作台：194 项原料、研发分流 107 项、65 条独立产品内编，支持双成本、配方新增与停用恢复、草稿试算启用及自动成本历史。2026-09-12 已按冻结现行配方补算采购 V1–V21，独立标为历史回算，原正式记录保留。详见 [研发工作台最新调整](./docs/研发工作台调整-2026-09-12.md)。

最近更新：[9月11—12日更新日志](./docs/更新日志-2026-09-11至12.md) · [当前交接](./docs/新对话交接-2026-09-11.md) · [知识收尾与清场预览](./docs/知识收尾-2026-09-12.md)
...
“我的账号”只读展示身份与已授权范围，保留修改密码；姓名、部门与权限统一在用户管理维护。左下角帮助入口提供使用说明。意见反馈支持问题、建议和截图，普通用户仅查看自己的记录，管理员负责处理。组织、反馈和界面本轮收尾状态见 [提交准备](./docs/提交准备-2026-09-10.md)。
...
价格台账默认列为：原料编号、库存量、单位、最新价格、库存价格、价格变化、价格日期、修改人、状态。库存数量保留报表精度，表格仅显示整数部分，鼠标悬停查看完整数值；点击库存量表头可按原始数值升降序排列，实际零库存或缺失显示 `—`，排序仍将零库存视为数值零。库存价格为空或为零显示 `—`。台账的价格变化直接比较本项正式价格与上一次正式价格；仅补充新原料时保留已有原料的价格与比较基准。价格相同显示 0.0%，首次定价或无法比较显示居中的灰底“暂无对比”。排序、筛选和原料详情使用同一口径。价格历史将纯补料单独标明，并仅列本次新增价格；已有原料详情不重复计入补料沿用记录。进入编辑保留库存量等原列顺序，在最新价格后插入待发布价。详见 [台账价格比较与补料历史](./docs/价格台账最近调价-2026-09-11.md)。
...
本机正式入口为 `http://127.0.0.1:8000/`，环境为 `production`，数据目录为 `%LOCALAPPDATA%\HonghaoAI\production`（Codex 宿主可能映射到自身 LocalCache）。2026-09-12 已重新核验健康、就绪及页面构建一致性；9月11—12日增量按本次用户授权纳入提交推送；不能用旧基线48f49c8恢复当前正式服务。当前交接、证据和待处理项见 [交接记录](./docs/新对话交接-2026-09-11.md)。

2026-09-09 首次发布的历史基线：PR #57 已合并，采购域整体取自 8012 验收库；当次保留旧库的项目、会话、消息、任务及原账号凭据，不包含 8020 的试操作。8012、8020 和旧 `.data` 不是现役正式库，暂留复核，等待清场确认。当次完整恢复包包含数据库、配置、采购原件、代码与构建产物，并完成独立恢复验证。该记录不代表此后的账号状态或最新构建，历史证据见 [发布审查记录](./docs/发布审查-2026-09-09.md)。端口本身不能证明环境性质。
...
正式环境禁止通过 `HONGHAO_MODULE_*_MODE` 绕过后台门禁。首次启用或启用边界修订后，管理员必须确认业务负责人、数据安全、代码审查和回退演练四项均已通过；已有有效记录的恢复启用仅需再次确认，不要求手填 Issue/PR 地址。审查确认随待生效配置保存，重启时再次校验，详见 [启用确认简化](./docs/启用确认简化-2026-09-10.md)。发布仍由管理员或 IT 使用审核后的版本标签手动完成；系统不提供一键发布。
...
采购资讯接入及依赖见 [采购资讯接入](./docs/采购资讯接入.md)。2026-09-10 用户验收并授权后，已部署至正式8000；原采购及账号数据不变，未提交或推送。部署检查和备份见 [采购资讯正式部署](./docs/采购资讯正式部署-2026-09-10.md)。
