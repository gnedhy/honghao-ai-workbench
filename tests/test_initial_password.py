...
def test_initial_password_defaults_and_reset(tmp_path):
    ...
    with authenticated_client(settings) as client:
        ...
        with sqlite3.connect(settings.database_path) as db:
            admin_before = db.execute('SELECT * FROM identity_users WHERE id=?', (admin['id'],)).fetchone()
            sessions = db.execute('SELECT * FROM identity_sessions WHERE user_id=?', (admin['id'],)).fetchall()
        ...
        with sqlite3.connect(settings.database_path) as db:
            assert db.execute('SELECT * FROM identity_users WHERE id=?', (admin['id'],)).fetchone() == admin_before
            assert db.execute('SELECT * FROM identity_sessions WHERE user_id=?', (admin['id'],)).fetchall() == sessions
            assert not db.execute('SELECT * FROM identity_sessions WHERE user_id=?', (user['id'],)).fetchall()
            salt, hashed = db.execute('SELECT password_salt,password_hash FROM identity_users WHERE id=?', (user['id'],)).fetchone()
