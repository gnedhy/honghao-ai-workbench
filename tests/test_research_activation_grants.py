...
def users_for(store):
    identity = IdentityStore(store.url)
...
def test_grant_manager_and_scope_boundaries(ready):
    ...
    grants.set_grant(store.url, admin, manager['id'], True, True, 'research')
    grants.set_grant(store.url, manager, editor['id'], True, scope='research')
    assert grants.capabilities(store.url, editor, 'research')['can_activate']
    assert not grants.capabilities(store.url, editor)['can_activate']
    assert not grants.capabilities(store.url, editor, 'research')['can_manage_catalog']
    for target, flag in [(other, None), (editor, True)]:
        with pytest.raises(PermissionError):
            grants.set_grant(store.url, editor, target['id'], True, flag, 'research')
    with pytest.raises(PermissionError):
        grants.set_grant(store.url, manager, other['id'], True, True, 'research')
    with pytest.raises(PermissionError):
        grants.set_grant(store.url, manager, manager['id'], False, scope='research')
    with pytest.raises(ValueError):
        grants.set_grant(store.url, manager, viewer['id'], True, scope='research')
    grants.set_grant(store.url, admin, other['id'], True)
    assert not grants.capabilities(store.url, other, 'research')['can_activate']
    grants.set_grant(store.url, admin, manager['id'], False, scope='research')
    assert grants.capabilities(store.url, manager, 'research')['can_activate']
    assert not grants.capabilities(store.url, manager, 'research')['can_manage_grants']
    assert len(grants.grants(store.url,'research')['events']) == 3
    assert len(grants.grants(store.url)['events']) == 1
...
def test_http_grant_activate_revoke_and_no_deactivation(ready,saved_by_editor):
    ...
    with TestClient(create_app(settings)) as client:
        ...
        grants.set_grant(store.url,admin,editor['id'],True,scope='research')
        ...
        grants.set_grant(store.url,admin,editor['id'],False,scope='research')
...
def test_account_change_invalidates_retained_grant(ready,change):
    ...
    assert not grants.capabilities(store.url,identity.get_user(editor['id']),'research')['can_activate']
    with pytest.raises(ValueError,match='权限已变化'):
        store.activate(K,receipt,editor['id'])
    assert next(u for u in grants.grants(store.url,'research')['users'] if u['id']==editor['id'])['granted']


def test_http_manager_can_assign_but_not_promote(ready):
    settings, _, admin_id, store = ready
    identity, (manager, editor, other, viewer) = users_for(store)
    grants.set_grant(store.url,identity.get_user(admin_id),manager['id'],True,True,'research')
    with TestClient(create_app(settings)) as client:
        ...
        with pytest.raises(ValueError,match='权限已变化'):
            grants.set_grant(store.url,manager,other['id'],True,scope='research')
...
def test_grant_history_pagination_names_and_stable_order(ready,scope,count):
    ...
    with transaction(store.url, write=True) as db:
        for index in range(count):
            db.execute(f"INSERT INTO {scope}_admin_events(id,actor_id,action,target_id,detail,created_at) VALUES(%s,%s,%s,%s,%s,%s)",(f'event-{index}',admin_id,'grant.enabled',editor['id'],json.dumps({'name':'保存时姓名'} if index%2 else {}),'2026-09-14T10:00:00+00:00'))
    first=grants.grants(store.url,scope)
    ...
    for offset in range(0,max(count,1),50):
        page=grants.grants(store.url,scope,offset,50)
...
def test_revoke_commits_before_waiting_activation_rechecks_permission(ready):
    from concurrent.futures import ThreadPoolExecutor
    from threading import Event

    _, _, admin_id, store = ready
    identity, (_, editor, _, _) = users_for(store)
    admin = identity.get_user(admin_id)
    grants.set_grant(store.url, admin, editor['id'], True, scope='research')
    _, receipt = saved(store, editor['id'])
    ...
    def activate_after_start():
        started.set()
        return store.activate(K, receipt, editor['id'])

    with ThreadPoolExecutor(max_workers=1) as executor:
        with transaction(store.url, write=True):
            grants.set_grant(store.url, admin, editor['id'], False, scope='research')
            future = executor.submit(activate_after_start)
