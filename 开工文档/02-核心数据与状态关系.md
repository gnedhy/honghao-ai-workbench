...
| 对象 | 作用 | 关键字段 |
|---|---|---|
| `Project` | 轻量上下文容器，用于聚合相关会话、任务和知识，不承担复杂项目管理 | `id`、`title`、`workspace_path`、`updated_at` |
| `Conversation` | 用户与个人智能体的一条持续对话，可选择关联项目 | `id`、`title`、`project_id?`、`mode`、`updated_at` |
...
| `Task` | 用户发起的业务任务，可选择继承项目上下文 | `id`、`title`、`project_id?`、`status`、`goal`、`workspace_path` |
...
项目只保存轻量关联和受控目录入口：会话、任务和知识可以关联或移除项目；Skill 与 Workflow 是企业共享能力，不归项目所有，只在运行时被任务调用。

```mermaid
flowchart LR
    P["轻项目上下文"] -.可选关联.-> V["Conversation"]
    P -.可选关联.-> B
    P -.来源标记.-> K1
